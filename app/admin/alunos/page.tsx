        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-20 space-y-3">
               ...
               <p className="text-sm text-muted-foreground">Tente ajustar sua busca ou cadastrar um novo aluno.</p>
                    ...
                    <TableRow key={aluno.id} className="admin-table-row">
                      <TableCell>
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground truncate max-w-[200px]">{aluno.cidade || 'Cidade não inf.'}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          <Badge variant="outline" className="w-fit text-[10px] font-normal">{aluno.carreira || 'Sem carreira'}</Badge>
                          <span className="text-xs text-muted-foreground">{aluno.ano || 'S/ Turma'} • {aluno.turno || 'S/ Turno'}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                         <div className="flex flex-col gap-1">
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1 max-w-[250px]">
                          {aluno.aluno_habilidade?.slice(0, 3).map((h: any, i: number) => (
                            <Badge key={i} variant="secondary" className="text-[10px] py-0 h-4">
                              {h.habilidade?.nome}
                            </Badge>
                          ))}
                          {aluno.aluno_habilidade?.length > 3 && (
                            <span className="text-[10px] text-muted-foreground">+{aluno.aluno_habilidade.length - 3}</span>
                          )}
                        </div>
                      </TableCell>
