...
import { ArrowLeft, BookOpen, Home, Menu, Globe, ShieldCheck, Info } from "lucide-react"
                    ...
                    {(active || hoveredPath === item.href) && (
                      <motion.div
                        ...
                        className={cn(
                          ...
                          active ? "bg-background shadow-sm" : "bg-background/40"
                        )}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <Button 
              ...
              className={cn(
                ...
                !isScrolled && "bg-background/50 backdrop-blur-md border border-white/10 hover:bg-background"
              )}
            >
                ...
                <div className="flex flex-col h-full">
                  <div className="p-8 border-b border-border/10 bg-muted/20">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-xl font-black tracking-tighter">Hub<span className="text-primary">TCC</span></div>
                        <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Inovação e Tecnologia</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-8 border-t border-border/10 bg-muted/20 space-y-6">
                    <div className="flex items-center justify-between bg-background/50 p-4 rounded-2xl border border-border/10">
                      <span className="font-bold text-sm">Modo de Visualização</span>
                      <ThemeToggle />
                    </div>
                    <Button asChild variant="outline" size="lg" className="w-full h-14 rounded-2xl font-bold border-2 hover:bg-primary hover:text-primary-foreground transition-all duration-500 group">
                      <a href={MAIN_SITE_URL} onClick={() => setIsMenuOpen(false)}>
