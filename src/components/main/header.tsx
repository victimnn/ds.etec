...
import { Menu, Globe, ShieldCheck, Home, BookOpen, GraduationCap, Briefcase, Mail, Info, ArrowRight, LayoutGrid } from "lucide-react"
...
export function Header() {
  ...
  const logoClass = mounted && theme === 'dark' ? "brightness-0 invert" : "logo-main-primary"
        ...
        <div className={cn(
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className="relative">
              ...
              <img
                src="/logo-ds.png"
                alt="DS Logo"
                className={cn("relative h-9 md:h-11 w-auto object-contain transition-transform duration-500 group-hover:scale-110", logoClass)}
              />
            </div>
            <div className="flex flex-col">
              <span className="text-sm md:text-lg font-bold tracking-tight text-foreground leading-none">
                DS<span className="text-primary">{cn(isScrolled ? "EtecJB" : "Etec")}</span>
              </span>
              <span className={cn(
                    ...
                    {(active || hoveredPath === item.href) && (
                      <motion.div
                        ...
                        className={cn(
                          ...
                          active ? "bg-background shadow-md" : "bg-background/60"
                        )}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
              ...
              <SheetContent side="right" className="w-full sm:w-[380px] p-0 bg-background/95 backdrop-blur-2xl border-l-border/20">
                ...
                <div className="flex flex-col h-full">
                  <div className="p-8 border-b border-border/10 bg-muted/20">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="text-xl font-black tracking-tighter">DS<span className="text-primary">Etec</span></div>
                        <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Educação e Carreira</div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="pt-4 mt-4 border-t border-border/10">
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-lg font-black tracking-tight">Hub de TCC</span>
                          </div>
                  ...
                  <div className="p-8 border-t border-border/10 bg-muted/20 space-y-6">
                    <div className="flex items-center justify-between bg-background/50 p-4 rounded-2xl border border-border/10">
                      <span className="font-bold text-sm">Preferências Visuais</span>
                      <ThemeToggle />
                    </div>
                    <Button asChild size="lg" className="w-full h-14 rounded-2xl font-bold shadow-glow" onClick={() => setIsMenuOpen(false)}>
