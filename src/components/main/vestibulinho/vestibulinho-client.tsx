...
import { 
  ...
  CheckCircle, ExternalLink, Download, ArrowRight, 
  GraduationCap, BookOpen, Target, Award, Sparkles,
  Search, HelpCircle
} from 'lucide-react'
...
export function VestibulinhoClient() {
  ...
  const cronograma = [
    ...
    { evento: "Matrícula 1ª Chamada", data: "19 a 20 de Janeiro", status: getStatus("19 a 20 de Janeiro") },
  ]

  const faqs = [
    { q: "Quais os requisitos?", a: "Ter concluído o Ensino Fundamental (9º ano) ou estar cursando em 2025." },
    { q: "Como é a prova?", a: "50 questões de múltipla escolha sobre Português, Matemática, Ciências, História e Geografia." },
    { q: "O curso é gratuito?", a: "Sim, totalmente gratuito. Sem taxas de matrícula ou mensalidades." },
    { q: "Duração do curso?", a: "3 anos em período integral (Matutino ou Vespertino)." }
  ]
        ...
        <div className="relative container mx-auto px-4 py-24 lg:py-36">
          ...
          >
            ...
            <p className="text-xl text-primary-foreground/90 leading-relaxed max-w-2xl mx-auto text-pretty">
              Sua carreira em tecnologia começa agora. Garanta sua vaga em uma das melhores escolas técnicas do estado.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
              <Button 
                size="lg" 
                onClick={() => window.open('https://vestibulinho.etec.sp.gov.br/home/', '_blank')}
              ...
              <Button 
                ...
                onClick={() => window.open('https://vestibulinho.etec.sp.gov.br/documentos/default.asp', '_blank')}
              >
          ...
          >
            {[
              ...
              <motion.div key={i} variants={fadeInUp} className="text-center space-y-1">
                ...
                <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">{card.d}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <section className="py-24 px-4">
        <div className="container mx-auto">
          <motion.div {...fadeInUp} className="text-center mb-20 space-y-4">
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight">Cronograma <span className="text-primary">Oficial</span></h2>
            <p className="text-muted-foreground">Não perca os prazos mais importantes do processo.</p>
          </motion.div>
            ...
            {cronograma.map((item, index) => (
              <motion.div 
                ...
                className={cn(
                  ...
                  item.status === 'concluido' ? "bg-muted/30 opacity-60" : "bg-card shadow-sm hover:shadow-lg hover:border-primary/30"
                )}
              >
                <div className={cn(
                  "w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-inner",
                  item.status === 'concluido' ? "bg-muted text-muted-foreground" : 
                  item.status === 'proximo' ? "bg-orange-500/10 text-orange-600 animate-pulse" : "bg-primary/10 text-primary"
                )}>
                  {item.status === 'concluido' ? <CheckCircle className="h-6 w-6" /> : <Clock className="h-6 w-6" />}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-lg leading-tight">{item.evento}</h3>
                  <p className="text-sm text-muted-foreground">{item.data}</p>
                </div>
                <Badge variant={item.status === 'concluido' ? 'secondary' : 'default'} className="hidden sm:flex">
        ...
        <div className="container mx-auto">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <motion.div {...fadeInUp} className="space-y-8">
              ...
              <h2 className="text-4xl md:text-5xl font-bold tracking-tight">Dúvidas <span className="text-primary">Frequentes</span></h2>
              <p className="text-lg text-muted-foreground leading-relaxed">
                Preparamos uma lista com as principais perguntas para ajudar você a ingressar no curso de Desenvolvimento de Sistemas.
              </p>
              <Button asChild variant="outline" className="rounded-full h-12 px-8">
                <Link href="/contato">Ainda tenho dúvidas <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </motion.div>
            ...
            >
              <Accordion type="single" collapsible className="space-y-4">
                {faqs.map((faq, i) => (
                  <AccordionItem key={i} value={`faq-${i}`} className="bg-background rounded-3xl border px-8 shadow-sm">
